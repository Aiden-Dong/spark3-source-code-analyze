...
/**
 * 给定输入的 [[InternalRow]]，评估 [[Expression]] 的 Java 源代码。
 * @param code 评估表达式所需的语句序列。如果 isNull 和 value 已经存在，或者不需要代码来评估它们（字面量），则应该是空字符串。
 * @param isNull 保存一个布尔值，表示表达式是否评估为 null。
 * @param value 评估结果的（可能是原始的）值的术语。如果 isNull 设置为 true，则无效。
 */
...
/**
 * 用于代码生成的上下文，跟踪一个可以传递到生成的 Java 函数中的对象列表。
 */
