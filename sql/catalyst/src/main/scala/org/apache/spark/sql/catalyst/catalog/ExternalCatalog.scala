...
/**
 * 系统目录的接口（包括函数、分区、表和数据库）。
 * 这仅用于非临时项目，实现必须是线程安全的，因为它们可能在多个线程中被访问。这是外部目录，因为它预计会与外部系统交互。
 * 当数据库不存在时，实现应抛出 [[NoSuchDatabaseException]]。
 *
 * ExternalCatalog是系统Catalog的接口，提供访问function、partition和database的一系列方法。
 * 只作用为非临时的item，它的实现类必须是线程安全的，因为它会同时被多个线程访问。
 * ExternalCatalog为Spark提供了和外部系统交互的能力。
 * ExternalCatalog有两个实现类：HiveExternalCatalog和InMemoryCatalog。
 */
