...
object SQLConf {
  ...
  val HIVE_METASTORE_PARTITION_PRUNING =
    buildConf("spark.sql.hive.metastorePartitionPruning")
      .doc("当设置为 true 时，一些谓词将被下推到 Hive 元数据存储中，以便更早地消除不匹配的分区。.")
      .version("1.5.0")
      .booleanConf
      .createWithDefault(true)
  ...
  val HIVE_MANAGE_FILESOURCE_PARTITIONS =
    buildConf("spark.sql.hive.manageFilesourcePartitions")
      .doc("当设置为 true 时，还可以为文件源表启用 Hive 元数据存储管理。这包括数据源表和转换后的 Hive 表。" +
           "启用分区管理后，数据源表将分区存储在 Hive 元数据存储中，并在查询计划期间使用元数据存储来剪枝分区，" +
        "当设置 ${HIVE_METASTORE_PARTITION_PRUNING.key} 为 true 时。")
      .version("2.1.1")
      .booleanConf
      .createWithDefault(true)
