...
/**
 * 一个由 Spark 会话使用的内部目录。
 * 这个内部目录充当底层元数据存储（例如 Hive 元数据存储）的代理，它还管理所属 Spark 会话的临时视图和函数。
 *
 * HiveExternalCatalog 为Spark提供了与Hive MetaStore的交互能力。
 * 具体实现方法是，HiveExternalCatalog会通过hadoop相关的配置文件来实例化一个ClientForMetadata，所有的元数据访问方法都是通过这个client来和hive交互。
 *
 * 这个类必须是线程安全的。
 */
private[spark] class HiveExternalCatalog(conf: SparkConf, hadoopConf: Configuration)
  ...
  /**
   * 用于与元数据存储交互的 Hive 客户端。
   */
  lazy val client: HiveClient = {
    HiveUtils.newClientForMetadata(conf, hadoopConf)
  }

  // 我们希望包装的由 Hive 客户端抛出的异常。
  ...
  /**
   * 这是否是应该被包装的由 Hive 客户端抛出的异常。
   * 由于类加载器隔离问题，在这里无法使用模式匹配，所以我们需要比较异常的规范名称，我们假设这些名称是稳定的。
   */
  ...
  /**
   * 在涉及 client 的代码中运行一些代码，并在过程中抛出特定异常时将其包装在 [[AnalysisException]] 中，使用 [[synchronized]] 块。
   */
  ...
  /**
   * 检查数据列名的有效性。Hive元数据存储禁止表使用一些特殊字符（','、':' 和 ';'）作为数据列名，包括嵌套列名。
   * 分区列没有此限制。
   * 视图也没有此限制。
   */
