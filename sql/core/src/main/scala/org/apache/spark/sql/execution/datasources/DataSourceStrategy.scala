...
object DataSourceAnalysis extends Rule[LogicalPlan] {
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan resolveOperators {
    ...
    case i @ InsertIntoStatement(
      ...
      // 如果 InsertIntoTable 命令用于分区的 HadoopFsRelation，并且用户指定了静态分区，
      // 则在查询之上添加一个 Project 运算符， 以在查询结果中包含这些常量列值。
      //
      // 例如：
      // 假设我们有一个表 "t"，通过以下方式创建：
      // CREATE TABLE t (a INT, b INT, c INT) USING parquet PARTITIONED BY (b, c)
      // "INSERT INTO TABLE t PARTITION (b=2, c) SELECT 1, 3"
      // 将转换为 "INSERT INTO TABLE t PARTITION (b, c) SELECT 1, 2, 3"。
      //
      // 基本上，我们会将具有分配值的分区列放回到 SELECT 子句中。
      // SELECT 子句的输出组织为
      // 普通列 静态分区列 动态分区列。
      // 静态分区列是在 PARTITION 子句中具有分配值的分区列（例如，上面示例中的 b）。
      // 动态分区列是在 PARTITION 子句中没有分配值的分区列（例如，上面示例中的 c）。
      ...
      // 对于动态分区的覆盖，我们不会提前删除分区目录。
      // 我们会先写入到临时目录，然后在写入作业完成后移动到最终的分区目录。
      // 因此，允许输出路径尝试覆盖输入路径是可以的。
      if (overwrite && !insertCommand.dynamicPartitionOverwrite) {  // 非动态分区
