...
object FileFormatWriter extends Logging {
  ...
  /**
   * 此命令的基本工作流程如下:
   * 1. 驱动程序端设置，包括输出提交器初始化和为要发出的写作业准备的数据源特定工作。
   * 2. 发出一个写作业，其中包含一个或多个执行器端任务，每个任务写入一个 RDD 分区中的所有行。
   * 3. 如果任务中没有抛出异常，则提交该任务，否则
   * 4. 如果任务中没有抛出异常，则提交该任务，否则中止该任务；如果在任务提交期间抛出任何异常，也会中止该任务。
   * 5. 如果所有任务都已提交，则提交作业，否则中止作业；如果在作业提交期间抛出任何异常，也会中止作业。
   * 6. 如果作业成功提交，则执行后提交操作，如处理统计信息。
  @return 在此写入作业期间更新的所有分区路径的集合。
   */
  def write(
